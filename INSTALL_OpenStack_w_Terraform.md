...
Before building anything, the build node / workstation from where Terraform and Ansible will run should be prepared.
...
1. Install required packages:
   ...
   ```
   sudo apt-get update
   sudo apt-get -y install unzip python-virtualenv python-pip python-dev sshpass git libffi-dev libssl-dev libyaml-dev vim graphviz python-pydot python-pydot-ng python-pyparsing libcdt5 libcgraph6 libgvc6 libgvpr2 libpathplan4
   ```
...
1. Download version of the terraform (latest or preferable):
...
4. [Optional] Enable autocompletion:
...
```
cd && git clone https://github.com/dataengi/ansible-hortonworks.git
```

If your GitHub SSH key is installed, you can use the SSH link:

```
cd && git git@github.com:dataengi/ansible-hortonworks.git
```


# Set the OpenStack variables

Modify the file at `terraform/live/stage/main.tf` to set the OpenStack configuration.


## cloud_config

[Set environment variables (in file terraform.tfvars, with -var flag, from environment variables)](https://www.terraform.io/intro/getting-started/variables.html)    
 E.g. contents of terraform.tfvars:
 
 username            = "userName"
 tenantname          = "tenantName"
 password            = "pass"
 openstack_auth_url  = "http://openstack.auth.url:5000/v3/"
 openstack_keypair   = "openstack-key-pair"
...
| nodescount            | The number of nodes to be built under this host_group. |
...
| network_name          | The name or ID of the internal network to be connected to. |                                                      |
| admin_username        | Username of sudo-user of installed image . |                                                      |
| keyname               | The name of a key pair to put on the server. The key pair must already be created and associated with the tenant's account. Changing this creates a new server. |                                                      |
| zone                  | The name of the OpenStack zone.                         |
| private_key           | Specify client private key file for SSH client authentication. You must specify a path to the file. |                                                      |
| enable_persist_volume | Enable persist volume . |                                                      |
| persist_volume_size   | Size in gigabytes of volume which should be created to store persistent data, e.g. 200 . |
...
Enter terraform build directory terraform/live/stage.
...
Result of terraform apply will be static inventory file inventory/static with created host groups and hosts. 
